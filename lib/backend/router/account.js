...
const hasSignedIn=require('../auth/has-signed-in.js');
const passwordService=require('../auth/password-service.js');
const sendEmailToActivate=require('../email/send-email-to-activate.js');
...
router.post("/signup",bodyParser.urlencoded({extended:true}),(req,res)=>{
    ...
    passwordService.generate(password)
        .then(password=>{
            signupService.signup(username,password,email,invitation)
                .then(
                    (info)=>{
                        console.log("注册成功：",info.username,req.ip);
...
router.get('/invitation',(req,res)=>{
    domain.activeCode.create({
        expiresAt:signupService.generateExpiresDate(2),
    }).then((activateCode)=>{
