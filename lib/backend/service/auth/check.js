 ...
 */
function checkLogin(redirect="/account") {
 ...
 */
function checkRole(ROLE_STR,redirect="/") {
 ...
 */
function checkAnyRole(ROLES_ARRAY,redirect="/") {

    return (req,res,next)=>{
        let roles=req.session.roles||[];
        let accessable=false;
        console.log(`检查权限：${roles} ,要求${ROLES_ARRAY} 之一`);
        for(let i=0;i<roles.length;i++){
            if(ROLES_ARRAY.indexOf(roles[i])!==-1){
                console.log(`检查到权限：${roles[i]} ,要求${ROLES_ARRAY}`);
                accessable=true;
                break;
            }
        }
 ...
 */
function checkAllRoles(ROLES_ARRAY,redirect="/") {
    return (req, res, next) => {
        let roles=req.session.roles||[];
        let accessable = true;
